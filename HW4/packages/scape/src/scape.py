...
class Control():
    ...
    def __init__(self):
        ...
        rospy.init_node('scape', anonymous=True)
        self.cmd_publisher = rospy.Publisher('/cmd_vel', Twist, queue_size=5)
                
        self.angular_k_i = rospy.get_param("/scape/k_i")
        self.angular_k_p = rospy.get_param("/scape/k_p")
        self.angular_k_d = rospy.get_param("/scape/k_d")
        
        self.dt = 0.005
        self.v = 0.5
        self.D = rospy.get_param("/scape/D") # m
        rate = 1/self.dt
